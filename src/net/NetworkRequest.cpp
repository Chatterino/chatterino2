...
namespace chatterino
{
    NetworkRequest::NetworkRequest(
        const std::string& url, NetworkRequestType requestType)
    ...
    void NetworkRequest::setRawHeader(
        const char* headerName, const QByteArray& value)
    {
        this->data->request_.setRawHeader(headerName, value);
    }

    void NetworkRequest::setRawHeader(
        const char* headerName, const QString& value)
    {
    ...
    void NetworkRequest::makeAuthorizedV5(
    ...
    void NetworkRequest::execute()
    {
        ...
        switch (this->data->requestType_)
        {
            ...
            {
                // Put requests cannot be cached, therefore the request is
                // called immediately
                this->doRequest();
            }
            break;
            ...
            {
                // Delete requests cannot be cached, therefore the request is
                // called immediately
                this->doRequest();
            }
            break;
    ...
    void NetworkRequest::doRequest()
    {
        ...
        auto onUrlRequested = [data = this->data, timer = this->timer,
            ...
            auto reply = [&]() -> QNetworkReply* {
                ...
                {
                    case NetworkRequestType::Get:
                        return NetworkManager::accessManager.get(
                            data->request_);

                    case NetworkRequestType::Put:
                        return NetworkManager::accessManager.put(
        ...
        QObject::connect(
            &requester, &NetworkRequester::requestUrl, worker, onUrlRequested);
