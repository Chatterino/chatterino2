...
namespace chatterino
{
    ...
    void Channel::addMessage(
        MessagePtr message, boost::optional<MessageFlags> overridingFlags)
    {
        auto app = getApp();
        MessagePtr deleted;
    ...
    void Channel::addOrReplaceTimeout(MessagePtr message)
    {
        ...
        for (int i = snapshotLength - 1; i >= end; --i)
        {
                ...
                s->timeoutUser == message->timeoutUser)  //
            {
                ...
                MessageBuilder replacement(
                    systemMessage, message->searchText + QString(" (") +
        ...
        for (int i = 0; i < snapshotLength; i++)
        {
            auto& s = snapshot[i];
            if (s->flags.hasNone(
                    {MessageFlag::Timeout, MessageFlag::Untimeout}) &&
    ...
    void Channel::disableAllMessages()
    {
        ...
        for (int i = 0; i < snapshotLength; i++)
        {
            ...
            // FOURTF: disabled for now
            const_cast<Message*>(message.get())
