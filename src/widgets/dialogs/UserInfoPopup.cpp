...
#    endif
        ...
        auto user = layout.emplace<QHBoxLayout>().withoutMargin();
        {
            ...
            QObject::connect(
                viewLogs.getElement(), &Button::leftClicked, [this] {
    ...
    void UserInfoPopup::installEvents()
    {
            ...
            this->ui_.follow, &QCheckBox::stateChanged, [this](int) mutable {
                ...
                if (this->ui_.follow->isChecked())
                {
                    currentUser->followUser(
                        this->userId_, reenableFollowCheckbox);
                }
                else
                {
        ...
        // ignore highlights
        QObject::connect(this->ui_.ignoreHighlights, &QCheckBox::clicked,
            [this](bool checked) mutable {
                ...
                else
                {
                    ...
                    for (int i = 0; i < vector.size(); i++)
                    {
                        ...
                        {
                            getApp()->highlights->blacklistedUsers.removeItem(
                                i);
                            i--;
                        }
                    }
                    if (getApp()->highlights->blacklistContains(
                            this->userName_))
                    {
        ...
        const auto onIdFetched = [this, hack](QString id) {
            ...
            currentUser->checkFollow(id, [this, hack](auto result) {
                if (hack.lock())
                {
                    ...
                    {
                        this->ui_.follow->setEnabled(true);
                        this->ui_.follow->setChecked(
                            result == FollowResult_Following);
                    }
                }
            });
            ...
            [&](const QString& title_,
                ...
                auto vbox = layout.emplace<QVBoxLayout>().withoutMargin();
                {
                    ...
                    for (const auto& item : items)
                    {
                        ...
                        {
                            a->setScaleIndependantSize(
                                buttonWidth2, buttonHeight);
                        }
                        else
                        {
                            a->setScaleIndependantSize(
                        ...
                        QObject::connect(a.getElement(),
                            &EffectLabel2::leftClicked,
                            [this, timeout = std::get<1>(item)] {
                                this->buttonClicked.invoke(
                                    std::make_pair(Action::Timeout, timeout));
                            });
                    }
                }
            };
