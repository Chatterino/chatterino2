...
namespace chatterino
{
    ...
    {
        auto layout = LayoutCreator<BrowserExtensionPage>(this)
                          .setLayoutType<QVBoxLayout>();

        auto label =
            layout.emplace<QLabel>("The browser extension will replace the "
                                   "default Twitch.tv chat with "
                                   "chatterino while chatterino is running.");
        label->setWordWrap(true);

        auto chrome =
            layout.emplace<QLabel>("<a href=\"" CHROME_EXTENSION_LINK
