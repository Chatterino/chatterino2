...
namespace chatterino
{
    ...
    QCheckBox* SettingsLayout::addCheckbox(
        const QString& text, BoolSetting& setting)
    {
    ...
    DescriptionLabel* SettingsLayout::addDescription(const QString& text)
    {
        ...
        label->setTextInteractionFlags(
            Qt::TextBrowserInteraction | Qt::LinksAccessibleByKeyboard);
    ...
    void GeneralPage::initLayout(SettingsLayout& layout)
    {
        ...
        layout.addDropdown<int>("Font size",
        ...
        layout.addDropdown<float>("Mouse scroll speed",
            {"0.5x", "0.75x", "Default", "1.5x", "2x"}, s.mouseScrollMultiplier,
            [](auto val) {
                if (val == 1)
                    return QString("Default");
                else
        ...
        layout.addDropdown("Timestamp format",
            {"h:mm", "hh:mm", "h:mm a", "hh:mm a"}, s.timestampFormat, true);
        layout.addDropdown<int>("Collapse messages",
        ...
        layout.addDropdown<float>("Emote size",
            {"0.5x", "0.75x", "Default", "1.25x", "1.5x", "2x"}, s.emoteScale,
            [](auto val) {
                if (val == 1)
                    return QString("Default");
                else
        ...
        layout.addDropdown("Emoji set",
            {"EmojiOne 2", "EmojiOne 3", "Twitter", "Facebook", "Apple",
                "Google", "Messenger"},
            s.emojiSet);
        ...
        layout.addCheckbox(
            "Show subscriber badges ", getSettings()->showBadgesSubscription);
        ...
        layout.addCheckbox(
            "Double click links to open", s.linksDoubleClickOnly);
        ...
        layout.addDropdown<int>("Historic messages appearance",
        ...
        layout.addDescription("Chatterino saves files on disk to speed up
        loading " "times and reduce network usage."); this->cachePath =
        layout.addDescription("%cachePath%"); layout.addDropdown("Cache
        directory", {"Automatic"});
        */
        ...
        layout.addDescription(createNamedLink(
            CHROME_EXTENSION_LINK, "Download for Google Chrome"));
        ...
        layout.addDropdown("Preferred quality", {"Choose", "Source", "High",
                                                 "Medium", "Low", "Audio
        only"}); layout.addDropdown("Command line arguments", {"..."});
        */
    }  // namespace chatterino
            ...
            getSettings()->cachePath.connect(
                [cachePath = this->cachePath](const auto&, auto) mutable {
                    ...
                    QString pathShortened =
                        "Current location: <a href=\"file:///" + newPath +
