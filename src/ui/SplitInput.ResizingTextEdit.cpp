...
namespace chatterino::ui
{
    ResizingTextEdit::ResizingTextEdit()
    {
        ...
        // possibly resize when text changed
        QObject::connect(
            this, &QTextEdit::textChanged, this, &QWidget::updateGeometry);

        // disable tabbing
        this->setFocusPolicy(Qt::ClickFocus);

        // try to remove all margins (added by qss later)
        this->setContentsMargins(0, 0, 0, 0);

        // only allow regular text
    ...
    void ResizingTextEdit::keyPressEvent(QKeyEvent* event)
    {
        ...
        if (doComplete)
        {
            ...
            {
                // First type pressing tab after modifying a message, we refresh
                // our completion model
        ...
        // (hemirt)
        // this resets the selection in the completion list, it should probably
        // only trigger on actual chat input (space, character) and not on every
        // key input (pressing alt for example) (fourtf) fixed for shift+tab,
        // there might be a better solution but nobody is gonna bother anyways
