...
namespace chatterino::ui
{
        ...
        , currentValueAnimation_(this, "currentValue_")
    {
        ...
        // misc stuff
        // this->resize(int(16 * this->scale()), 100);  // TODO: remove
    ...
    void Scrollbar::replaceHighlight(
        size_t index, ScrollbarHighlight replacement)
    {
    ...
    {
        animated &= true;  // getSettings()->enableSmoothScrolling.getValue();
        value = std::max(this->minimum_,
            std::min(this->maximum_ - this->largeChange_, value));

        if (std::abs(this->desiredValue_ + this->smoothScrollingOffset_ -
                     value) > 0.0001)
        {
            if (animated)
            {
            ...
            else
            {
                    ...
                    QPropertyAnimation::Running)
                {
                    ...
                    this->currentValueAnimation_.stop();
                    this->atBottom_ =
                        ((this->getMaximum() - this->getLargeChange()) -
                            value) <= 0.0001;
        ...
        emit this->desiredValueChanged(value);
    }
    ...
    {
        value = std::max(
            this->minimum_, std::min(this->maximum_ - this->largeChange_,
        ...
        if (std::abs(this->currentValue_ - value) > 0.0001)
        {
            ...
            this->updateScroll();
            emit this->currentValueChanged(value);
    ...
    void Scrollbar::paintEvent(QPaintEvent*)
    {
        ...
        // painter.fillRect(rect(), QColor("#666"));
        ...
        {
            painter.fillRect(this->thumbRect_, QColor("#777"));
        }
        // mouse not over thumb
        else
        {
            painter.fillRect(this->thumbRect_, QColor("#888"));
        }
        ...
        for (size_t i = 0; i < snapshotLength; i++)
        {
            ...
            if (!highlight.isNull())
            {
                ...
                switch (highlight.getStyle())
                {
                    ...
                    {
                        painter.fillRect(
                            w / 8 * 3, int(y), w / 4, highlightHeight, color);
                    }
                    break;
    ...
    void Scrollbar::mouseMoveEvent(QMouseEvent* event)
    {
        ...
        else if (this->mouseDownIndex_ == 2)
        {
            ...
            this->setDesiredValue(this->desiredValue_ + qreal(delta) /
                                                            this->trackHeight_ *
                                                            this->maximum_);
        }
    ...
    void Scrollbar::updateScroll()
    {
        ...
        this->thumbRect_ = QRect(0,
            int(this->currentValue_ / this->maximum_ * this->trackHeight_) + 1 +
                this->buttonHeight_,
            this->width(),
