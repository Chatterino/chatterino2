...
namespace chatterino
{
    ...
    // methods
    void MessageLayoutContainer::begin(
        int width, float scale, MessageFlags flags)
    {
        this->clear();
        this->width_ = width;
        this->scale_ = scale;
    ...
    void MessageLayoutContainer::_addElement(
        MessageLayoutElement* element, bool forceAdd)
    {
        ...
        element->setPosition(QPoint(
            this->currentX_, this->currentY_ - element->getRect().height()));
    ...
    void MessageLayoutContainer::breakLine()
    {
        ...
        if (this->flags_.has(MessageFlag::Centered) &&
            this->elements_.size() > 0)
        {
        ...
        for (size_t i = lineStart_; i < this->elements_.size(); i++)
        {
            ...
            {
                // yExtra -= (this->textLineHeight_ -
                // element->getRect().height()) / 2;
            }
        ...
        this->height_ =
            this->currentY_ + int(this->margin.bottom * this->scale_);
        this->lineHeight_ = 0;
        this->line_++;
    }
    ...
    bool MessageLayoutContainer::fitsInLine(int _width)
    {
        return this->currentX_ + _width <=
               (this->width_ - int(this->margin.left * this->scale_) -
                   ...
                   (this->line_ + 1 == MAX_UNCOLLAPSED_LINES
    ...
    void MessageLayoutContainer::end()
    {
        ...
        {
            static TextElement dotdotdot(
                "...", MessageElementFlag::Collapsed, MessageColor::Link);
            static QString dotdotdotText("...");

            auto* element = new TextLayoutElement(dotdotdot, dotdotdotText,
    ...
    void MessageLayoutContainer::paintAnimatedElements(
        QPainter& painter, int yOffset)
    {
        for (const std::unique_ptr<MessageLayoutElement>& element :
            this->elements_)
        {
        ...
        QPainter& painter, int messageIndex, Selection& selection, int yOffset)
    {
            ...
            selection.selectionMax.messageIndex > messageIndex)
        {
            for (Line& line : this->lines_)
            {
                ...
                rect.setBottom(
                    std::min(this->height_, rect.bottom()) + yOffset);
                rect.setLeft(
                    this->elements_[line.startIndex]->getRect().left());
                rect.setRight(
                ...
                for (int i = line.startIndex; i < line.endIndex; i++)
                {
                    ...
                    if (index + c > selection.selectionMin.charIndex)
                    {
                                ...
                                /*=*/selection.selectionMax.charIndex)  //
                        {
                            returnAfter = true;
                            index = line.startCharIndex;
                            for (int i = line.startIndex; i < line.endIndex;
                                 i++)
                            {
    ...
    int MessageLayoutContainer::getSelectionIndex(QPoint point)
    {
        ...
        int lineStart = line == this->lines_.end()
                            ? this->lines_.back().startIndex
                            : line->startIndex;
        if (line != this->lines_.end())
        {
            line++;
        }
        int lineEnd = line == this->lines_.end() ? this->elements_.size()
                                                 : line->startIndex;
        ...
        for (int i = 0; i < lineEnd; i++)
        {
            ...
            // this is the word
            auto rightMargin =
                element->hasTrailingSpace() ? this->spaceWidth_ : 0;
    ...
    void MessageLayoutContainer::addSelectionText(
