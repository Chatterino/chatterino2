...
namespace chatterino
{
    namespace detail
    {
            ...
            while (true)
            {
                ...
                if (this->durationOffset_ > this->items_[this->index_].duration)
                {
                    this->durationOffset_ -=
                        this->items_[this->index_].duration;
        ...
        template <typename Assign>
        auto makeConvertCallback(
            const QVector<Frame<QImage>>& parsed, Assign assign)
        {
    ...
    Image::Image(const QPixmap& pixmap, qreal scale)
        : scale_(scale)
        , frames_(
              std::make_unique<detail::Frames>(QVector<detail::Frame<QPixmap>>{
                  detail::Frame<QPixmap>{pixmap, 1}}))
    {
    }
    ...
    void Image::load()
    {
        ...
        req.onSuccess([that = this, weak = weakOf(this)](
                          auto result) -> Outcome {
