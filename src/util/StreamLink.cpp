...
namespace chatterino
{
    namespace
    {
        ...
        bool checkStreamlinkPath(const QString& path)
        {
            ...
            {
                return false;
                // throw Exception(fS("Streamlink path ({}) is invalid, file
                // does not exist", path));
            }
        ...
        void showStreamlinkNotFoundError()
        {
            ...
            if (getSettings()->streamlinkUseCustomPath)
            {
                msg->showMessage("Unable to find Streamlink executable\nMake "
                                 ...
                                 "path "
                                 "is pointing "
                                 "to the DIRECTORY where the streamlink "
            ...
            else
            {
                msg->showMessage("Unable to find Streamlink executable.\nIf "
                                 "you have Streamlink "
                                 "installed, you might need to enable the "
                                 "custom path option");
            }
        }
    ...
    void getStreamQualities(
        const QString& channelURL, std::function<void(QStringList)> cb)
    {
            ...
            [=](int res) {
                ...
                if (lastLine.startsWith("Available streams: "))
                {
                    ...
                    for (int i = split.length() - 1; i >= 0; i--)
                    {
                        ...
                        if (option == "best)")
                        {
                            // As it turns out, sometimes, one quality option
                            // can be the best and worst quality at the same
                            // time. Since we start loop from the end, we can
        ...
        QStringList extraArguments)
    {
        ...
        bool res = QProcess::startDetached(
            getStreamlinkProgram() + " " + QString(arguments.join(' ')));
