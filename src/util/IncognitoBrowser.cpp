...
namespace chatterino
{
    namespace
    {
        ...
        {
            // list of command line switches to turn on private browsing in
            // browsers
        ...
        QString getCommand(const QString& link)
        {
            ...
            auto command = QSettings(
                "HKEY_CLASSES_ROOT\\" + browserId + "\\shell\\open\\command",
                QSettings::NativeFormat)
                               .value("Default")
                               .toString();
            if (command.isNull())
