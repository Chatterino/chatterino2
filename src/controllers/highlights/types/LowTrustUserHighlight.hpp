...
struct LowTrustUserHighlight {
    ...
    static constexpr QStringView DEFAULT_NAME = u"Low Trust User";
    ...
    Outcome outcome = Outcome::create<LowTrustUserHighlight>();

    HighlightCheck buildCheck() const;

    friend QDebug operator<<(QDebug dbg, const LowTrustUserHighlight &v);
};
...
template <>
struct Serialize<chatterino::highlights::LowTrustUserHighlight> {
    using H = chatterino::highlights::LowTrustUserHighlight;
...
template <>
struct Deserialize<chatterino::highlights::LowTrustUserHighlight> {
    using H = chatterino::highlights::LowTrustUserHighlight;
