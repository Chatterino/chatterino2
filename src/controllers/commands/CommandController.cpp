...
namespace chatterino
{
    void CommandController::initialize(Settings&, Paths& paths)
    {
        ...
        // Add loaded commands to our vector of commands (which will update the
        // map of commands)
        ...
        const QString& textNoEmoji, ChannelPtr channel, bool dryRun)
    {
        ...
        if (!dryRun && channel->isTwitchChannel())
        {
            if (commandName == "/w")
            {
                ...
                for (int i = 2; i < words.length(); i++)
                {
                    ...
                    {  // emoji/text
                            ...
                            app->emotes->emojis.parse(words[i]))
                        {
                            ...
                            {
                                void operator()(
                                    EmotePtr emote, MessageBuilder& b) const
                                {
                                    b.emplace<EmoteElement>(
                ...
                auto overrideFlags =
                    boost::optional<MessageFlags>(messagexD->flags);
        ...
        const QStringList& words, const Command& command)
    {
        ...
        while (true)
        {
            ...
            result += command.func.mid(
                lastCaptureEnd, match.capturedStart() - lastCaptureEnd + 1);
