...
namespace chatterino
{
    ...
    public:
        ...
        bool isMatch(const QString& subject) const
        {
            return !this->pattern_.isEmpty() &&
                   (this->isRegex()
                           ...
                           : subject.contains(
                                 this->pattern_, this->caseSensitivity()));
        }
        ...
        {
            return this->isCaseSensitive_ ? Qt::CaseSensitive
                                          : Qt::CaseInsensitive;
        }
        ...
        bool containsEmote() const
        {
            if (!this->emotesChecked_)
            {
                ...
                for (const auto& acc : accvec)
                {
                    ...
                    {
                        if (this->replace_.contains(
                                emote.first.string, Qt::CaseSensitive))
                        {
...
namespace pajlada
{
    ...
    struct Serialize<chatterino::IgnorePhrase>
    {
            ...
            rapidjson::Document::AllocatorType& a)
        {
            ...
            chatterino::rj::set(
                ret, "caseSensitive", value.isCaseSensitive(), a);
    ...
    struct Deserialize<chatterino::IgnorePhrase>
    {
        static chatterino::IgnorePhrase get(const rapidjson::Value& value)
        {
            if (!value.IsObject())
            {
                return chatterino::IgnorePhrase(QString(), false, false,
            ...
            return chatterino::IgnorePhrase(
                _pattern, _isRegex, _isBlock, _replace, _caseSens);
        }
    };
