...
namespace chatterino
{
    void NotificationController::initialize(Settings& settings, Paths& paths)
    {
        ...
        this->channelMap[Platform::Twitch].delayedItemsChanged.connect(
            [this] {  //
    ...
    NotificationModel* NotificationController::createModel(
        QObject* parent, Platform p)
    {
    ...
    {
        auto i = std::find(
            fakeTwitchChannels.begin(), fakeTwitchChannels.end(), channelName);
        if (i != fakeTwitchChannels.end())
        {
