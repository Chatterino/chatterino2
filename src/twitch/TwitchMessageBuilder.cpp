...
#include "irc/IrcHelpers.hpp"
#include "messages/Message.hpp"
#include "twitch/TwitchBadges.hpp"
#include "twitch/TwitchEmotes.hpp"
...
namespace chatterino
{
    namespace
    {
        ...
        inline std::vector<RawTwitchEmote> parseRawEmotesSorted(
            const QString& emotes, const QString& originalMessage)
        {
            qDebug() << "emotes" << emotes;

            std::vector<RawTwitchEmote> result;

            // example: 25:4-8,14-18/77:88-102,14-18
            for (auto&& emote : emotes.splitRef('/'))
            {
                if (emote.isEmpty())
                    continue;

                auto [id_, occurences] = splitPairRef(emote, ':');

                for (auto&& occurence : occurences.split(','))
                {
                    auto [startStr, endStr] = splitPairRef(occurence, '-');
                    auto start = startStr.toInt();
                    auto end = endStr.toInt();

                    // check the range
                    if (start < 0 || start > end ||
                        end > originalMessage.length() + 1)
                        continue;

                    EmoteName name = {
                        originalMessage.mid(start, end - start + 1)};
                    EmoteId id = {id_.toString()};

                    if (!name.string.isEmpty())
                    {
                        result.push_back({start,
                            twitchEmotes().getOrCreateEmote(id, name), name});
                    }
                }
            }
    ...
    TwitchMessageBuilder::TwitchMessageBuilder(Room* _channel,
        ...
        : channel(_channel)
        , twitchChannel(dynamic_cast<TwitchRoom*>(_channel))
    ...
    TwitchMessageBuilder::TwitchMessageBuilder(Room* _channel,
        const Communi::IrcMessage* _ircMessage, const MessageParseArgs& _args,
    ...
    void TwitchMessageBuilder::initialize()
    {
        this->isPastMsg_ = this->tags.contains("historical");
        ...
        this->parseMessageID();
        this->parseRoomID();
        this->parseHighlights(this->isPastMsg_);
    ...
    MessagePtr TwitchMessageBuilder::build()
    {
        ...
        auto removeEmotesInRange =
            [](int pos, int len,
                ...
                auto it = std::partition(twitchEmotes.begin(),
            ...
            for (const auto& word : words)
            {
                for (const auto& emote : phrase.getEmotes())
                {
                    if (word == emote.first.string)
                    {
                        ...
                        twitchEmotes.push_back(
                            std::tuple<int, EmotePtr, EmoteName>{
                                startIndex + pos, emote.second,
        emote.first});
                    }
                }
                pos += word.length() + 1;
        ...
        twitchEmotes); auto mid = this->originalMessage_.mid(from, len);
                    ...
                    for (auto& tup : vret)
                    {
                        ...
                        if (match.hasMatch())
                        {
                            ...
                            {
                                std::get<0>(tup) = from +
        match.capturedStart(); twitchEmotes.push_back(std::move(tup));
                            }
                        }
                    }
            ...
            else
            {
                            ...
                            pattern, from, phrase.caseSensitivity())) != -1)
                {
                    ...
                    auto vret = removeEmotesInRange(from, len,
        twitchEmotes); auto replace = phrase.getReplace();
        ...
        for (const auto& word : words)
        {
                ...
                std::get<0>(*currentTwitchEmote) == i)
            {
                ...
                this->emplace<EmoteElement>(
                    emoteImage, MessageElementFlag::TwitchEmote);
    ...
    void TwitchMessageBuilder::addTextOrEmoji(const QString& string_)
    {
        this->emplace<TextElement>(
            string_, MessageElementFlag::Text, QColor("#fff"));
        ...
        if (linkString.isEmpty())
        {
            if (string.startsWith('@'))
            {
                this->emplace<TextElement>(string,
                    MessageElementFlag::BoldUsername, textColor,
            ...
            {
                this->emplace<TextElement>(
                    string, MessageElementFlag::Text, textColor);
            }
        }
        else
        {
            ...
            auto linkMEOriginal =
                this->emplace<TextElement>(
                        string, MessageElementFlag::OriginalLink, textColor)
                    ->setLink(link);

            /*LinkResolver::getLinkInfo(linkString,
                [weakMessage = this->weakOf(), linkMELowercase,
               linkMEOriginal, linkString](QString tooltipText, Link
               originalLink) { auto shared = weakMessage.lock(); if
               (!shared)
                    {
                        return;
                    }
        ...
        //        QRegularExpression::CaseInsensitiveOption);
        //        QRegularExpression getDomain("\\/\\/([^\\/]*)"); QString
        //        tempString = string;
    ...
    void TwitchMessageBuilder::parseRoomID()
    {
        ...
        if (auto it = this->tags.find("room-id"); it != this->tags.end())
        {
            this->roomID_ = it->toString();  // it.value().toString(); TODO: ???
            this->twitchChannel->setUserId(this->roomID_);
        }
    }
    ...
    void TwitchMessageBuilder::appendChannelName()
    {
        ...
        this->emplace<TextElement>(channelName,
    ...
    void TwitchMessageBuilder::parseUsername()
    {
        ...
        {
            this->userName =
                this->tags.value(QLatin1String("login")).toString();
        }
    ...
    void TwitchMessageBuilder::appendTimestamp()
    {
        // time from irc tags
        if (this->isPastMsg_)
        {
            ...
            if (auto ts = this->tags.value("tmi-sent-ts").toLongLong(&ok); ok)
            {
    ...
    {
        // user is neither a mod, staff, or broadcaster
        if (!args.isStaffOrBroadcaster && !this->senderIsBroadcaster &&
            this->tags.value("user-type").toString() != "mod")
        {
    ...
    void TwitchMessageBuilder::appendUsername()
    {
        ...
        if (auto dn = this->tags.find("display-name").value(); !dn.isNull())
        {
            QString displayName = parseTagString(dn.toString()).trimmed();

            if (QString::compare(
                    displayName, this->userName, Qt::CaseInsensitive) == 0)
            {
        ...
        else if (this->args.isReceivedWhisper)
        {
            ...
            this->emplace<TextElement>(usernameText,
                    MessageElementFlag::Username, this->usernameColor_,
                    FontStyle::ChatMediumBold)
    ...
    void TwitchMessageBuilder::appendContent()
    {
        auto emotes = parseRawEmotesSorted(
            this->tags["emotes"].toString(), this->originalMessage_);
        auto currentEmote = emotes.begin();
        auto index = 0;

        qDebug() << emotes.size();

        // add words one by one
        for (auto&& word : this->originalMessage_.split(' '))
        {
            // twitch emote
            if (currentEmote != emotes.end() && currentEmote->index == index)
            {
                this->emplace<EmoteElement>(
                    currentEmote->emote, MessageElementFlag::TwitchEmote);
    ...
    void TwitchMessageBuilder::parseHighlights(bool isPastMsg)
    {
        ...
        // TODO: This vector should only be rebuilt upon highlights being
        // changed fourtf: should be implemented in the HighlightsController
        ...
        if (getSettings()->enableSelfHighlight && currentUsername.size() >
        0)
        {
            HighlightPhrase selfHighlight(currentUsername,
        ...
        if (!app->highlights->blacklistContains(this->ircMessage->nick()))
        {
            for (const HighlightPhrase& highlight : activeHighlights)
            {
                ...
                {
                    log("Highlight because {} matches {}",
                        this->originalMessage_, highlight.getPattern());
                    doHighlight = true;
                    ...
                    {
                        // Break if no further action can be taken from
        other
                        // highlights This might change if highlights can
        have
    ...
    /*void TwitchMessageBuilder::appendTwitchEmote(
        const QString& emote, std::vector<RawTwitchEmote>& vec)
    {
        ...
        for (auto&& occurence : parameters.at(1).split(','))
        {
            ...
            auto name =
                EmoteName{this->originalMessage_.mid(start, end - start +
    1)}; auto tup = std::tuple<int, EmotePtr, EmoteName>{ start,
    app->emotes->twitch.getOrCreateEmote(id, name), name}; if
    ...
    void TwitchMessageBuilder::appendTwitchBadges()
    {
        ...
        if (auto it = this->tags.find("badges"); it != this->tags.end())
        {
            for (auto&& badge : it.value().toString().splitRef(','))
            {
                ...
                // global badge
                if (auto badge = twitchBadges().badge(set, version))
                {
                    qDebug() << "badge" << set;
                ...
                {
                    QString cheerAmount = badge.mid(5).toString();
                    ...
                    try
                    {
                        if (twitchChannel)
                            if (const auto& badge =
                                    this->twitchChannel->twitchBadge(
                                        "bits", cheerAmount))
                            {
                    ...
                    {
                        // Channel does not contain a special bit badge for this
                        // version
                    }

                    // Use default bit badge
                    if (auto badge =
                            this->twitchChannel->globalTwitchBadges().badge(
                                "bits", cheerAmount))
                    {
                        this->emplace<EmoteElement>(
                                badge.get(), MessageElementFlag::BadgeVanity)
                ...
                {
                    if (auto customModBadge =
                            this->twitchChannel->ffzCustomModBadge())
                    {
                        this->emplace<EmoteElement>(customModBadge.get(),
                ...
                else if (badge.startsWith("partner/"))
                {
                    ...
                    switch (index)
                    {
                        ...
                        default:
                        {
                            printf("[TwitchMessageBuilder] Unhandled partner
                badge " "index: %d\n", index);
                        }
                        break;
                    }
                ...
                else
                {
                    ...
                    if (auto badgeEmote =
                            this->twitchChannel->twitchBadge(splits[0],
                splits[1]))
                    {
                        this->emplace<EmoteElement>(
                                badgeEmote.get(),
    ...
    void TwitchMessageBuilder::appendChatterinoBadges()
    {
        ...
        //{
        //    this->emplace<EmoteElement>(
        //        *chatterinoBadgePtr, MessageElementFlag::BadgeChatterino);
        //}
    }
    ...
    Outcome TwitchMessageBuilder::tryParseCheermote(const QString& string)
    {
        ...
        // app->resources->channels[this->roomID_]; if
        // (channelResources.loaded)
        // {
        //    for (const auto &cheermoteSet :
        //    channelResources.cheermoteSets) {
        ...
        //            Log("Error getting a cheermote from a cheermote set
        //            for the bit amount {}",
        //                numBits);
        //            return Failure;
        //        }
        ...
        //        this->emplace<TextElement>(amount,
        //        MessageElementFlag::Text, cheermote.color);
