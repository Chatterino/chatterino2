...
namespace chatterino
{
    ...
    TwitchServer::TwitchServer()
        : whispersChannel(
              new Channel("/whispers", Channel::Type::TwitchWhispers))
        , mentionsChannel(
              new Channel("/mentions", Channel::Type::TwitchMentions))
        , watchingChannel(Channel::getEmpty(), Channel::Type::TwitchWatching)
    {
        ...
        // getSettings()->twitchSeperateWriteConnection.connect([this](auto,
        // auto) { this->connect(); },
        //                                                     this->signalHolder_,
        //                                                     false);
    }
    ...
    void TwitchServer::initializeConnection(
        IrcConnection* connection, bool isRead, bool isWrite)
    {
        ...
        connection->sendCommand(Communi::IrcCommand::createCapability(
    ...
    std::shared_ptr<Channel> TwitchServer::createChannel(
    ...
    void TwitchServer::privateMessageReceived(
    ...
    void TwitchServer::writeConnectionMessageReceived(
        Communi::IrcMessage* message)
    {
        switch (message->type())
        {
            ...
            {
                IrcMessageHandler::getInstance()
                    .handleWriteConnectionNoticeMessage(
                        static_cast<Communi::IrcNoticeMessage*>(message));
            }
            break;
        ...
        const QString& channelName)
    {
        ...
        if (channelName == "$$$")
        {
            static auto channel = std::make_shared<Channel>(
                "$$$", chatterino::Channel::Type::Misc);
            static auto timer = [&] {
                for (auto i = 0; i < 1000; i++)
                {
                    channel->addMessage(
                        makeSystemMessage(QString::number(i + 1)));
                }
    ...
    void TwitchServer::onMessageSendRequested(
        TwitchChannel* channel, const QString& message, bool& sent)
    {
...

        {
            ...
            auto& lastMessage = channel->hasModRights()
