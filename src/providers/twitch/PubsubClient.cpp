...
namespace chatterino
{
    ...
    namespace detail
    {
        PubSubClient::PubSubClient(
            WebsocketClient& websocketClient, WebsocketHandle handle)
        ...
        bool PubSubClient::send(const char* payload)
        {
            ...
            this->websocketClient_.send(
                this->handle_, payload, websocketpp::frame::opcode::text, ec);
    ...
    PubSub::PubSub()
    {
        ...
        this->moderationActionHandlers["subscribers"] =
            [this](const auto& data, const auto& roomID) {
                ModeChangedAction action(data, roomID);
        ...
        const QString& channelID, std::shared_ptr<TwitchAccount> account)
    {
        ...
        std::string topic(
            fS("chat_moderator_actions.{}.{}", userID, channelID));
    ...
    void PubSub::listenToTopic(
        const std::string& topic, std::shared_ptr<TwitchAccount> account)
    {
    ...
    void PubSub::onMessage(
        websocketpp::connection_hdl hdl, WebsocketMessagePtr websocketMessage)
    {
        ...
        else if (type == "PONG")
        {
            ...
            // If this assert goes off, there's something wrong with the
            // connection creation/preserving code KKona
    ...
    void PubSub::onConnectionOpen(WebsocketHandle hdl)
    {
        ...
        // We separate the starting from the constructor because we will want to
    ...
    void PubSub::handleMessageResponse(const rapidjson::Value& outerData)
    {
        ...
        else if (topic.startsWith("chat_moderator_actions."))
        {
            ...
            {
                log("Missing moderation action in data: {}",
                    rj::stringify(data));
                return;
            }
            ...
            {
                log("No handler found for moderation action {}",
                    moderationAction);
                return;
            }
