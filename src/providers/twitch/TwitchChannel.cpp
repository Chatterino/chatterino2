...
namespace chatterino
{
    namespace
    {
            ...
            const QJsonObject& jsonRoot, ChannelPtr channel)
        {
            ...
            for (const auto jsonMessage : jsonMessages)
            {
                ...
                // passing nullptr as the channel makes the message invalid but
                // we don't check for that anyways
                auto message = Communi::IrcMessage::fromData(content, nullptr);
                auto privMsg =
                    dynamic_cast<Communi::IrcPrivateMessage*>(message);
                assert(privMsg);
        ...
        std::pair<Outcome, UsernameSet> parseChatters(
            const QJsonObject& jsonRoot)
        {
            static QStringList categories = {
                "moderators", "staff", "admins", "global_mods", "viewers"};
    ...
    TwitchChannel::TwitchChannel(const QString& name,
        TwitchBadges& globalTwitchBadges, BttvEmotes& bttv, FfzEmotes& ffz)
    ...
    {
        BttvEmotes::loadChannel(this->getName(),
            [this, weak = weakOf<Channel>(this)](auto&& emoteMap) {
                if (auto shared = weak.lock())
                    this->bttvEmotes_.set(
                        std::make_shared<EmoteMap>(std::move(emoteMap)));
            });
        FfzEmotes::loadChannel(this->getName(),
            [this, weak = weakOf<Channel>(this)](auto&& emoteMap) {
                if (auto shared = weak.lock())
                    this->ffzEmotes_.set(
                        std::make_shared<EmoteMap>(std::move(emoteMap)));
    ...
    void TwitchChannel::sendMessage(const QString& message)
    {
        ...
        {
            // XXX: It would be nice if we could add a link here somehow that
            // opened the "account manager" dialog
        ...
        this->sendMessageSignal.invoke(
            this->getName(), parsedMessage, messageSent);
        ...
        if (!this->joinedUsersMergeQueued_)
        {
            ...
            QTimer::singleShot(500, &this->lifetimeGuard_, [this] {
                ...
                MessageBuilder builder(
                    systemMessage, "Users joined: " + joinedUsers->join(", "));
    ...
    void TwitchChannel::addPartedUser(const QString& user)
    {
        ...
        if (!this->partedUsersMergeQueued_)
        {
            ...
            QTimer::singleShot(500, &this->lifetimeGuard_, [this] {
                ...
                MessageBuilder builder(
                    systemMessage, "Users parted: " + partedUsers->join(", "));
                builder->flags.set(MessageFlag::Collapsed);
                this->addMessage(builder.release());
                partedUsers->clear();
    ...
    boost::optional<EmotePtr> TwitchChannel::bttvEmote(
    ...
    boost::optional<EmotePtr> TwitchChannel::ffzEmote(
        const EmoteName& name) const
    {
    ...
    Outcome TwitchChannel::parseLiveStatus(const rapidjson::Document& document)
    {
...

        {
            ...
            {
                const auto& broadcastPlatformValue =
    ...
    void TwitchChannel::refreshPubsub()
    {
        ...
        getApp()->twitch2->pubsub->listenToChannelModerationActions(
            roomId, account);
    }
    ...
    void TwitchChannel::refreshBadges()
    {
        ...
        req.onSuccess([this, weak = weakOf<Channel>(this)](
                          auto result) -> Outcome {
            auto shared = weak.lock();
            if (!shared)
                return Failure;
                 ...
                 jsonBadgeSet++)
            {
                     ...
                     jsonVersion_++)
                {
                    ...
                    auto emote = std::make_shared<Emote>(Emote{EmoteName{},
                        ImageSet{
                            Image::fromUrl(
    ...
    void TwitchChannel::refreshCheerEmotes()
    {
        ...
        + this->getRoomId()}; auto request =
        NetworkRequest::twitchRequest(url.string);
        request.setCaller(QThread::currentThread());

        request.onSuccess(
            [this, weak = weakOf<Channel>(this)](auto result) -> Outcome {
                auto cheerEmoteSets =
        ParseCheermoteSets(result.parseRapidJson()); std::vector<CheerEmoteSet>
        emoteSets;
                ...
                for (auto &set : cheerEmoteSets) {
                    ...
                    for (auto &tier : set.tiers) {
                        ...
                        // We will continue to do so for now since we haven't
        had to
