...
void IrcMessageHandler::handleRoomStateMessage(Communi::IrcMessage *message)
{
    ...
    auto chan = getApp()->twitch2->getChannelOrEmpty(chanName);
...
void IrcMessageHandler::handleUserStateMessage(Communi::IrcMessage *message)
{
    ...
    auto c = getApp()->twitch2->getChannelOrEmpty(channelName);
    if (c->isEmpty())
    {
        return;
    }
...
void IrcMessageHandler::handleWhisperMessage(Communi::IrcMessage *message)
{
    ...
    auto c = getApp()->twitch2->whispersChannel.get();
    ...
    getApp()->twitch2->lastUserThatWhisperedMe.set(builder.userName);

    if (_message->flags.has(MessageFlag::Highlighted))
    {
        getApp()->twitch2->mentionsChannel->addMessage(_message);
    }
...
void IrcMessageHandler::handleNoticeMessage(Communi::IrcNoticeMessage *message)
{
    ...
    for (const auto &msg : builtMessages)
    {
            ...
            channelName == "jtv")
        {
            ...
            // channels
            getApp()->twitch2->forEachChannelAndSpecialChannels(
                [msg](const auto &c) {
                    c->addMessage(msg);
                });
        ...
        auto channel = getApp()->twitch2->getChannelOrEmpty(channelName);
...
{
    auto channel = getApp()->twitch2->getChannelOrEmpty(
