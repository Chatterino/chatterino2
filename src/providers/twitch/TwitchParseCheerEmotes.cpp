...
namespace chatterino
{
    namespace
    {
        ...
        inline bool ReadValue(
            const rapidjson::Value& object, const char* key, Type& out)
        {
        ...
        template <>
        inline bool ReadValue<QString>(
            const rapidjson::Value& object, const char* key, QString& out)
        {
            if (!object.HasMember(key))
            {
                return false;
        ...
        template <>
        inline bool ReadValue<std::vector<QString>>(
            const rapidjson::Value& object, const char* key,
        ...
        inline bool ParseSingleCheermoteSet(
            JSONCheermoteSet& set, const rapidjson::Value& action)
        {
            if (!action.IsObject())
            {
                return false;
    ...
    // https://api.twitch.tv/kraken/bits/actions?channel_id=11148817 for
    // cheermote sets or "Actions" as they are called in the API
    std::vector<JSONCheermoteSet> ParseCheermoteSets(
