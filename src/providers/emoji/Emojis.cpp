...
namespace chatterino
{
    namespace
    {
            ...
            QString shortCode = QString())
        {
            ...
            rj::getSafe(
                unparsedEmoji, "non_qualified", emojiData->nonQualifiedCode);
            ...
            rj::getSafe(
                unparsedEmoji, "has_img_emojione", capabilities.emojione);
            rj::getSafe(
                unparsedEmoji, "has_img_facebook", capabilities.facebook);
            rj::getSafe(
                unparsedEmoji, "has_img_messenger", capabilities.messenger);
    ...
    void Emojis::loadEmojis()
    {
        ...
        rapidjson::ParseResult result =
            root.Parse(data.toUtf8(), data.length());
        ...
        for (const auto& unparsedEmoji : root.GetArray())
        {
            ...
            if (unparsedEmoji.HasMember("skin_variations"))
            {
                    ...
                    unparsedEmoji["skin_variations"].GetObject())
                {
                    ...
                    this->shortCodes.push_back(
                        variationEmojiData->shortCodes[0]);

                    this->emojiFirstByte_[variationEmojiData->value.at(0)]
                        .append(variationEmojiData);

                    this->emojis.insert(
                        variationEmojiData->unifiedCode, variationEmojiData);
                }
            }
        }
    }
    ...
    void Emojis::sortEmojis()
    {
        ...
        {
            std::stable_sort(
                p.begin(), p.end(), [](const auto& lhs, const auto& rhs) {
    ...
    void Emojis::loadEmojiSet()
    {
        getSettings()->emojiSet.connect([=](const auto& emojiSet) {
            ...
            static std::map<QString, QString> emojiSets = {
                ...
                emoji->emote =
                    std::make_shared<Emote>(Emote{EmoteName{emoji->value},
                        ImageSet{Image::fromUrl({url}, 0.35)},
        ...
        const QString& text)
    {
        ...
        for (auto i = 0; i < text.length(); ++i)
        {
            ...
            if (charactersFromLastParsedEmoji > 0)
            {
                ...
                result.emplace_back(text.mid(
                    lastParsedEmojiEndIndex, charactersFromLastParsedEmoji));
            }
