...
namespace chatterino
{
    namespace
    {
            ...
            const QString& tooltip, Emote& emoteData)
        {
            ...
            //, code, tooltip
            emoteData.name = name;
            emoteData.images = ImageSet{Image::fromUrl(url1x, 1),
                Image::fromUrl(url2x, 0.5), Image::fromUrl(url3x, 0.25)};
            ...
            const QJsonObject& jsonRoot, const EmoteMap& currentEmotes)
        {
            ...
            {
                auto jsonEmotes =
                    jsonSet.toObject().value("emoticons").toArray();
                ...
                for (auto jsonEmoteValue : jsonEmotes)
                {
                    ...
                    emote.homePage = Url{
                        QString("https://www.frankerfacez.com/emoticon/%1-%2")
                            .arg(id.string)
                            .arg(name.string)};
