...
namespace chatterino
{
    ...
    void Toasts::sendChannelNotification(const QString& channelName, Platform p)
    {
        ...
        if (p == Platform::Twitch)
        {
            ...
            {
                this->fetchChannelAvatar(channelName,
    ...
    void Toasts::sendWindowsNotification(const QString& channelName, Platform p)
    {
        ...
        std::wstring imagePath =
            std::wstring(temp_Utf8.begin(), temp_Utf8.end());
        ...
        WinToastLib::WinToast::instance()->setAppUserModelId(
            WinToastLib::WinToast::configureAUMI(
                L"", L"Chatterino 2", L"", aumi_version));
        ...
        const QString channelName, std::function<void(QString)> successCallback)
    {
        ...
        request.onSuccess([successCallback](auto result) mutable -> Outcome {
            ...
            {
                // log("API Error while getting user id, users is not an
                // array");
                successCallback("");
                return Failure;
            }
