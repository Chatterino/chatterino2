...
GlobalShortcutResult wrapWinError(BOOL success)
{
    if (success == TRUE)
    {
    ...
    return {.ok = false, .error = GetLastError()};
}
...
quint32 GlobalShortcutPrivate::nativeModifiers(Qt::KeyboardModifiers modifiers)
{
...
quint32 GlobalShortcutPrivate::nativeKeycode(Qt::Key key)
{
    switch (key)
    {
        case Qt::Key_Escape:
