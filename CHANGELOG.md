...
- Major: Newly uploaded Twitch emotes are now once again present in the emote picker, and can be autocompleted with Tab. (#2992)
- Minor: Added autocompletion in /whispers for Twitch emotes, Global Bttv/Ffz emotes, and emojis. (#2999, #3033)
- Minor: Received Twitch messages will now use the exact same timestamp (obtained from Twitch's server) for every Chatterino user instead of assuming the message timestamps on the client's side. (#3021)
...
- Major: Added a username autocompletion popup menu when typing usernames with an @ prefix. (#1979, #2866)
- Major: Added the ability to toggle the visibility of Channel Tabs - This can be done by right-clicking the tab area or pressing the keyboard shortcut (default: Ctrl+U). (#2600)
- Minor: Usernames in the channel points reward redemption messages are now clickable. (#2673, #2953)
- Minor: Channel names in the  `<channel> has gone offline. Exiting host mode.` message are now clickable. (#2922)
- Minor: Added the `/openurl` command. Usage: `/openurl <URL>`. Opens the provided URL in the browser. (#2461, #2926)
- Minor: Updated to Emoji v13.1 (#2958)
- Minor: Added the options to "Open in: New Tab, Browser Player, StreamLink" in the twitch link context menu. (#2988)
- Minor: Sender usernames in the automod messages shown to moderators now show the correct color and display names. (#2967)
- Minor: The /live split now shows channels going offline. (#2880)
- Minor: Restored the automod functionality for moderators (#2817, #2887)
- Minor: Added a setting for username styles (#2889, #2891)
- Minor: Searching for users in the viewer list now searches anywhere in the user's name. (#2861)
- Minor: Added moderation buttons to the search popup when searching in a split with moderation mode enabled. (#2148, #2803)
- Minor: Made the "#channel" in `/mentions` tab show in usercards and in the search popup. (#2802)
- Minor: Added settings to disable custom FrankerFaceZ VIP/mod badges. (#2693, #2759)
- Minor: Limits the number of recent chatters to improve memory usage and reduce freezes. (#2796, #2814)
- Minor: Added the `/popout` command. Usage: `/popout [channel]`. It opens browser chat for the provided channel. Can also be used without arguments to open current channels browser chat. (#2556, #2812)
- Minor: Improved matching of game names when using the `/setgame` command (#2636)
- Minor: Now shows the deletion of messages like timeouts (#1155, #2841, #2867, #2874)
- Minor: Added a link to the accounts page in settings to "You need to be logged in to send messages" message. (#2862)
- Minor: Switched to Twitch v2 emote API for animated emote support. (#2863)
- Bugfix: Now deleting cache files that weren't modified in the past 14 days. (#2947)
- Bugfix: Fixed large timeout durations in moderation buttons caused by overlapping with usernames or other buttons. (#2865, #2921)
- Bugfix: Middle mouse click no longer scrolls in not fully populated usercards and splits. (#2933)
- Bugfix: Fixed the bad behavior of the HTML color picker edit when user input is being entered. (#2942)
- Bugfix: Made so the follower emotes (in the emote popup menu, tab completion, emote input menu) will only show up in their origin channel, and not globally. (#2951)
- Bugfix: Fixed the founder badge not being respected by `author.subbed` filter. (#2971)
...
- Bugfix: Fixed an issue with pasting text that included URLs (#1688, #2855)
